<script lang="ts">
    /**
     * Primary-reading-language picker, reachable from ReaderTopBar — needed
     * because the app's own outer header (with its React LanguageButton) is
     * now hidden while the reader is showing (see BaseLayout.astro's
     * data-page-type toggle), so the reader needs its own way to reach the
     * same functionality. Modeled directly on ParallelLeftLangSelector.svelte
     * (itself a Svelte-native near-clone of LanguageSelectorIsland.tsx) —
     * same data source, same quick-picks/search UX — but picking a language
     * here changes the app's PRIMARY reading language, which (unlike the
     * parallel view's left-language, a client-only display choice) requires
     * an actual navigation: the current book/chapter's content is resolved
     * per-ISO at the URL/build level, not swappable in place. Navigation
     * logic ported from LanguageButtonIsland.tsx's handleClose (mode
     * "primary" branch) — a plain window.location.href assignment (full
     * reload), not client-side routing.
     */
    ...
    import {
        ...
        $secondaryLanguages as secondaryLanguagesStore,
        $languageNames as languageNamesStore,
        loadLanguageNames
    } from '../../stores/language-store';
    import { buildLangHref } from '../bw/url-utils';
    ...
    function isSelected(code: string) {
        return code === $selectedLanguageStore;
    }

    function navigateToLang(lang: string) {
        ...
        window.location.href = buildLangHref(lang, restPath, $secondaryLanguagesStore);
    }

    function handleSelect(lang: Lang) {
        addRecentLang(lang.code);
        onClose();
        if (lang.code !== $selectedLanguageStore) {
            navigateToLang(lang.code);
        }
    }
...
<div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onclick={onClose}>
    ...
    >
        <div class="flex items-center justify-between p-4 border-b" style="border-color: var(--border, #e5e7eb)">
            <h2 class="text-lg font-semibold" style="color: var(--text)">{tr('languageSelector.title')}</h2>
            <button
                type="button"
                onclick={onClose}
                class="text-xl opacity-60 hover:opacity-100"
                style="color: var(--text)"
                aria-label={tr('languageSelector.close')}
            >
                &times;
            </button>
        </div>
        ...
        <div class="px-3 pb-3">
            <input
                ...
                placeholder={tr('languageSelector.searchPlaceholder')}
            ...
            <div class="flex-1 overflow-y-auto px-3 pb-3" bind:this={listEl}>
                ...
                {#if searchResults.length === 0}
                    <div class="text-center py-4" style="color: var(--text); opacity: 0.5">
                        {tr('languageSelector.noLangsFound')} "{searchTerm}"
                    </div>
                {/if}
            </div>
        {/if}
